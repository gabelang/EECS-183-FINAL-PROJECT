...
void Game::playGame(bool isAIModeIn, ifstream& gameFile) {
    ...
    while (gameFile >> temp) {
        if (!gameFile.fail()) {
            Person p(temp);
            while (p.getTurn() > building.getTime()) {
...
bool Game::isValidPickupList(const string& pickupList, const int pickupFloorNum) const {
    int duplicate;
    bool goingUp;
    //Establishes the direction of the elevator by passenger 0's direction.
    if (pickupFloorNum < building.getFloorByFloorNum(pickupFloorNum).getPersonByIndex(pickupList[0]).getTargetFloor()) {
        goingUp = true;
    }
    else {
        goingUp = false;
    ...
    if (pickupList.length() <= ELEVATOR_CAPACITY) {
        for (int i = 0; i < pickupList.length(); i++) {
            if (pickupList[i] - '0' < 0 || pickupList[i] - '0' > building.getFloorByFloorNum(pickupFloorNum).getNumPeople()) {
                return false;
            }
            //Checks the direction of every passenger compared to passenger 0
            //If the targetFloor is the same as the pickupFloor, it will report it as a bad move when it shouldn't even have gotten here.
            //The move class should handle a problem like this, but it's worth knowing in case it fails.
            if (goingUp && pickupFloorNum > building.getFloorByFloorNum(pickupFloorNum).getPersonByIndex(pickupList[i] - '0').getTargetFloor()) {
                return false;
            }
            else if (!goingUp && pickupFloorNum < building.getFloorByFloorNum(pickupFloorNum).getPersonByIndex(pickupList[i] - '0').getTargetFloor()) {
                return false;
            }
